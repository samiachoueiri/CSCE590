...
# Function to compute the projection matrix onto the column space of A
def projection_matrix(A):
    ATA_inv = np.linalg.inv(np.dot(A.T, A))  # (A^T A)^{-1}
    PA = np.dot(A, np.dot(ATA_inv, A.T))  # A (A^T A)^{-1} A^T
    return PA

# Gradient descent function with diagnostics and adjustments
def gradient_descent(A, b, lambda_, learning_rate, max_iter=2000, tolerance=1e-5):
...
print("Final cost J(x):", final_cost)

# Compute the projection matrix
basis = np.array([[1, 0], [0, 1], [0,0]])
projection_matrix_ = projection_matrix(basis)
x2d = np.dot(projection_matrix_, x_optimal)
print(x2d)
